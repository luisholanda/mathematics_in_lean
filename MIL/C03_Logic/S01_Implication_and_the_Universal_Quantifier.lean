...
theorem my_lemma4 :
  ...
  calc
    |x * y| = |x| * |y| := abs_mul _ _
    _ ≤ |x| * ε := mul_le_mul (by rfl) ylt.le (abs_nonneg y) (abs_nonneg x)
...
example (nnf : FnLb f 0) (nng : FnLb g 0) : FnLb (fun x ↦ f x * g x) 0 := by
  intro x
  rw [← zero_mul 0]
  exact mul_le_mul (nnf x) (nng x) (by rfl) (nnf x)
...
theorem Subset.trans : r ⊆ s → s ⊆ t → r ⊆ t := fun hrs hst _ h ↦ hst (hrs h)
